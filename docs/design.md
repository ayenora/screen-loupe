...
- **Sandbox:** on in every build, with the hardened runtime, so the download and the Mac App Store are one app with one data container. `ScreenLoupe.entitlements` grants only the sandbox and read-write access to files the user picks in an open or save panel; there is no network entitlement. Anyone can check this with `codesign -d --entitlements -`. Settings and the project live in the app's container (`~/Library/Containers/com.ayenora.screenloupe`). The screenshot folder chosen in Settings is only where the save panel opens, so it needs no bookmark: the panel grants access to the file saved.
...
- **Ruler:** `CornerRuler` (pure, tested) keeps the ruler either in drawable pixels, fixed in the Viewer, or pinned in source pixels with the arm lengths the user set; its placement is always whole source pixels. An arm never looks shorter than 32 pt: the shortest arm in source pixels is recomputed from the zoom, and a pinned ruler keeps its set lengths to return to. `RulerController` hit-tests and drags it; the overlay draws it. An unpinned ruler is fixed in the Viewer but snapped to source pixels, so under a moving image it would jump from cell to cell. On a zoom/pan change it is first fixed exactly where it was drawn (`CornerRuler.place`), then drawn at that free, unsnapped place (`CornerRuler.freePlacement`) with dimmed lengths; 0.12 s after the last change it eases onto its snapped placement (ease-out, 0.12 s), the drawing interpolated between the two. It never hides, so a ruler held while two fingers pan stays under the finger. The pin button and move band stay 0.6 s after the pointer leaves. Its corner is clamped into the viewport, since the drawable-pixel position saved on one display can lie beyond the Viewer on another. Presses go to the ruler first, then to reference layers, then to panning.
...
ScreenLoupe/
  ...
  Resources/    Assets.xcassets (the app icon, drawn by scripts/make_icon.swift); Info.plist is generated from build settings
  ScreenLoupe.entitlements   the sandbox and user-selected files, nothing else
ScreenLoupeTests/
...
17. Turn the ruler on, move it, stretch and flip an arm, pin it, zoom and pan: a pinned ruler stays on its pixels and keeps its lengths; an unpinned one stays in place without hiding or jumping and eases onto the pixels when the image stops. Hold the ruler's line with one finger and pan with two: it stays under the finger.
...
1. The Mac App Store as a second channel, after the notarized download on GitHub Releases that comes first. The build is already sandboxed (§4); what is left is the store listing and review.

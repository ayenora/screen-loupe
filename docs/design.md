...
- **Capture Area:** a borderless, non-opaque `NSPanel` at `.statusBar` level that joins all Spaces, so it stays on top across Spaces and full-screen apps.
  - The interior is fully transparent. **Clicks inside go through to the app underneath**, so hover states in Figma or Safari stay live. The frame therefore carries explicit handles:
    ...
    - **Position box and pin:** the L T R B box sits right of the frame, or left when it would leave the screen, and fades in with the tab. A square pin button sits right of the tab and the raise button beyond it (both left of the tab at the screen's edge, the pin next to it); a pinned frame hit-tests only these two, so it can't be moved, resized or nudged, and hides its band and handles. The raise button calls `WindowManager.showViewer`, which activates the app and orders the Viewer front.
...
- **Keep Viewer on Top:** a pin toggle at the right of the Viewer toolbar and a checkmarked item in the Window menu and the menu bar item. On, the Viewer is a `.floating` window that joins every Space, full-screen apps' included (`.canJoinAllSpaces` + `.fullScreenAuxiliary`, like the Capture Area frame): it stays above other apps' windows while they are active, and still below the Capture Area frame (`.statusBar`). A window that joins other apps' full-screen Spaces can't be full screen itself, so it drops `.fullScreenPrimary` meanwhile, and turning the pin on in full screen leaves full screen first and applies on `windowDidExitFullScreen`. A regular app's window doesn't enter another app's full-screen Space even so (the non-activating Capture Area panel does): only an accessory app's does, so `Settings.hasDockIcon` drops the Dock icon while the pin is on. A window that stops joining every Space stays in the Space showing, so unpinning over another app's full screen orders the Viewer out first, gives that app the focus, and brings the Viewer back on the first `activeSpaceDidChangeNotification` that shows an ordinary Space. There is no public API for Spaces: `NSScreen.otherAppInFullScreen` reads the windows on screen — no Dock window over the display (it is on every ordinary Space), normal-level windows of one other app only, one of them spanning the display's width down to its bottom (TextMate leaves the menu bar's strip free; Obsidian doesn't). Anywhere else the app keeps the focus it had. macOS can deactivate an app a moment after its activation policy changes, so `AppController` asks whether it was active before the switch, activates it after, and for half a second undoes a deactivation with `activate(ignoringOtherApps:)`: by then another app is active, and a cooperative `activate()` is refused. Unpinned, the Viewer drops from `.floating` back among ordinary windows, under any window ordered front meanwhile, so it orders itself front while the app is active. The choice is persisted.
...
27. Put an app in full screen (the green button) on the Viewer's display and turn on Keep on Top: the Viewer floats over it and is on every Space; Enter Full Screen is off meanwhile. The Dock icon is gone meanwhile. Turn it off there: the Viewer disappears from the full-screen app and is back on the desktop. With the Viewer itself full screen, turning it on first leaves full screen.

## 8. Open

1. Distribution: Developer ID with a notarized download, the Mac App Store, or both.

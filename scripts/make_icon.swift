#!/usr/bin/env swift
// Draws the app icon (mockup variant A, "loupe on graphite") and writes the macOS AppIcon set.
//
//   swift scripts/make_icon.swift ScreenLoupe/Resources/Assets.xcassets/AppIcon.appiconset
//
// Every size is drawn from the vector description, not downscaled from 1024 px. Coordinates below
// are on Apple's 1024 icon grid with a top-left origin, as in the mockup.
...
/// The magnified pixels in the lens: a small blue button with white text pixels on a light UI.
let pixelArt: [[UInt32]] = {
...
func drawIcon(in ctx: CGContext) {
    ...
    // Body: graphite squircle with a soft drop shadow.
    let body = CGPath(
    ...
    ctx.addPath(body)
    ctx.setFillColor(color(0x16181C))
    ctx.fillPath()
    ctx.restoreGState()
    ...
    let gradient = CGGradient(
        colorsSpace: CGColorSpace(name: CGColorSpace.sRGB), colors: [color(0x3A3F47), color(0x16181C)] as CFArray,
        locations: [0, 1])!
    ctx.drawLinearGradient(gradient, start: CGPoint(x: 0, y: 100), end: CGPoint(x: 0, y: 924), options: [])
    ctx.restoreGState()

    // Lens contents: magnified pixels.
    ...
    for (row, colors) in pixelArt.enumerated() {
        for (column, hex) in colors.enumerated() {
            ...
            ctx.fill(CGRect(x: 222 + CGFloat(column) * 92, y: 222 + CGFloat(row) * 92, width: 88, height: 88))
        }
    }
    ctx.restoreGState()
    ...
    // Handle: a darker grip with a light collar next to the rim.
...
guard arguments.count == 2 else {
    FileHandle.standardError.write("usage: make_icon.swift <AppIcon.appiconset directory>\n".data(using: .utf8)!)
    exit(1)
}
...
for slot in slots {
    ...
    try renderPNG(pixels: slot.points * slot.scale, to: directory.appendingPathComponent(name))
    images.append([
...
print("Wrote \(slots.count) icons to \(directory.path)")
